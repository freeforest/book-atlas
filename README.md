...
The ordinary library list now loads 200 deterministic rows at a time, discloses “已显示数量/总数量”, and offers an accessible, keyboard-operable load-more/retry action. Search, filtering, sorting, and catalog mutations restart from a consistent first page; a failed next page preserves the rows already shown. Cross-module focus and create/edit/merge refreshes preserve an explicitly requested book UUID even when it lies beyond the first page: the catalog returns the normal bounded page plus at most one primary-key-filtered target, which the UI identifies separately without loading intervening pages. A deleted, missing, or newly excluded target clears the selection and reports a redacted recoverable state instead of substituting the first row. It does not silently stop at 500 or put all 10,000 rows into SwiftUI state.

The latest Prompt 10 closure run built Debug and Hardened Runtime Release products, passed 188/188 unit/integration/migration/security/performance tests and 35/35 actually executed macOS UI tests, and added three-run fixed-fictional existing-library launch, database-open, 200-row first-page, next-page, disclosed multi-page scrolling/hitch, tag-count, and Schema 1–4→5 measurements. The measured Debug launch processes only reopen an already-created temporary Schema 5 database; data generation is outside the timing window and invalid performance arguments cannot fall back to the production database. Exact UUID focus from the graph and create/edit/merge refreshes now preserves an existing page-out target with one bounded filtered primary-key lookup and never substitutes an unrelated first-row book. Release Instruments launch measurement remains unverified because the measured desktop trace could not be authorized from this execution surface. These are one-host regression observations, not cross-device promises. A real manual VoiceOver and Accessibility Inspector traversal could not be completed from the current execution surface; Light/Dark, non-default accent, 520×360, Reduce Motion, spoken focus/order, and Inspector findings therefore remain explicit unverified release gates. Prompt 10 still awaits independent review.
