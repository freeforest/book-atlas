...
The accepted scope is book CRUD, local query and organization, deterministic duplicate review/merge, versioned CSV import with mapping and preview, Markdown/CSV export, full SQLite backup/restore, a bounded local relationship graph, and user-initiated external reading entries. Prompt 7 passed its third independent review at baseline `b27318c741fee5b4a66e5ad99cb979177285fef5`; Prompt 8 passed its second independent review at baseline `6ae90dd50ee71f574e0b4cc1ffccfd7e4c2e71aa`; Prompt 9 passed independent review at baseline `1f7a35cda11fcafd23aacab0cb5c72e811327d0b`. Prompt 10 passed independent acceptance at documentation baseline `ec0b04f1c004ef5c897d3269e335c92034d6021e`, against verified code baseline `4cc20b8c88cb674a4f9a52d3e8de70c295169281`. Prompts 0–10 are complete; the later explicitly authorized Prompt 11A manual-relation loop has passed controller functional acceptance and remains unreleased, with user Git and complete pending-change review PENDING. Prompt 11B is now separately authorized for BookKind and detail completeness; the newly authorized two-test precondition repair passed, alongside the unchanged historical 11 passing tests; the historical UI batch remains 1 passed / 1 failed, followed by a separately authorized accessibility repair passing its single UI and Release checks; the user confirmed all four fictional-memory manual checks. The controller accepted P11A/P11B's agreed functional scope on 2026-09-10; P11 functional work is complete and unreleased. There is no network client entitlement, AI duplicate detector, automatic merge, cloud backup, directory scanner, or whole-library graph.
...
unverified. Prompt 11B is now separately authorized; see
[its implementation and verification record](PLANS/PROMPT-11B.md).
Dated failures and stop records below
...
## 2026-09-07 — UI continuation on the frozen lifecycle-test state

The user supplied fresh explicit unlocked/interactive-session confirmation, no manual input/application switching and no concurrent UI automation. The 63-file frozen manifest remained identical before UI, after directed UI and after full UI: `36f3e76d74b2049f97674c23598e459093f3688a3e34dbea997add5a5f02b2ab`. No source/test/config edits. Prior 45/45 directed and 221/221 complete non-UI results belong to this same state and were not rerun. Host was macOS 26.5.2 (25F84), Xcode 26.6 (17F113), arm64 destination; no input-method, permission or system-setting change was made.

The following commands each ran once with independent fresh DerivedData and result paths. `$UI_EVIDENCE_DIR` substitutes the exact temporary root retained in the local handoff:

```sh
xcodebuild test -project BookAtlas.xcodeproj -scheme BookAtlas -configuration Debug -destination 'platform=macOS,arch=arm64' -parallel-testing-enabled NO -maximum-parallel-testing-workers 1 -derivedDataPath "$UI_EVIDENCE_DIR/relations-dd" -resultBundlePath "$UI_EVIDENCE_DIR/relations.xcresult" -only-testing:BookAtlasUITests/BookAtlasUITests/testManualRelationSubmittedSaveDisablesEditingCancelAndEscape -only-testing:BookAtlasUITests/BookAtlasUITests/testManualRelationEmptyStateKeyboardCancellationAndEscapeOwnership -only-testing:BookAtlasUITests/BookAtlasUITests/testManualRelationFilteredCounterpartUsesExactFocusAndClearFlow -only-testing:BookAtlasUITests/BookAtlasUITests/testManualRelationKeyboardCreateNavigateConfirmedDeleteAndGraphRefresh > "$UI_EVIDENCE_DIR/relations.log" 2>&1
xcodebuild test -project BookAtlas.xcodeproj -scheme BookAtlas -configuration Debug -destination 'platform=macOS,arch=arm64' -parallel-testing-enabled NO -maximum-parallel-testing-workers 1 -derivedDataPath "$UI_EVIDENCE_DIR/full-ui-dd" -resultBundlePath "$UI_EVIDENCE_DIR/full-ui.xcresult" -only-testing:BookAtlasUITests > "$UI_EVIDENCE_DIR/full-ui.log" 2>&1
```

Each command ran in a separate session, immediately captured `$?` in `bookatlas_exit`, wrote its own `<batch>-exit.txt`, and exited with that value. For each result, `xcrun xcresulttool get test-results summary --path <result> --compact` and `xcrun xcresulttool get test-results tests --path <result> --compact` separately exited 0 and retained complete JSON.

| Gate | Executed / passed / failed / skipped | Original process exit | Summary / tree parser exits |
| --- | --- | --- | --- |
| Four relation UI cases | 4 / 4 / 0 / 0 | 0 | 0 / 0 |
| Full BookAtlasUITests | 45 / 44 / 1 / 0 | 65 | 0 / 0 |

Directed tree identities exactly matched the four selectors; full tree identities exactly matched the frozen 45-name source list, preserving every original 44 case plus the busy-save case. XCUI actually ran; UI target compilation included DEBUG. No repetition, skipped case, active cancellation, split-suite substitution or workload change. Busy-save passed both directed and full runs; it proves normal Save entering a non-writing wait and disabled editing/save/cancel, native Escape retaining the editor/draft/direction, and no premature status/error. It does not prove database commit or rollback, and test-app termination is not cancellation of a real write. Ten-thousand-book scrolling and subsequent ordinary AX access also passed, but do not override the full-suite failure or establish a historical root cause.

The only failed identity was `BookAtlasUITests/testZeroResultSearchShowsFocusedIssueAndClearRecoversLibrary()`, source line 493. UTC+08: start 17:29:07.125, clear-filter click 17:29:31.763, assertion failure 17:29:44.803, suite end 17:29:48.560. The failed predicate expected the restored count text “已显示 200 本，共 501 本，可以继续加载”. Later list-existence and selection-warning-disappearance assertions did not fail. Actual count-label value is not present in the extracted evidence; root cause remains UNKNOWN. Do not infer missing input, permission failure or common cause with historical failures.

Only this failure's `test-details` and `activities` were additionally parsed, each exit 0, using its exact test ID. Short selected activities and the relevant source/log window were inspected; no whole AX tree or unrelated system logs were output. A companion Xcode debugger-version warning is retained in the raw log without causal attribution.

**BLOCKED**: the single full UI budget is consumed. No second run, code repair, Release or incremental product audit occurred. Release/fixture-exclusion/new product entitlement evidence remains NOT VERIFIED because its prerequisite failed; Intel runtime NOT VERIFIED. Human QA, manual Git and complete pending-change scope remain PENDING. Old causes remain separately UNKNOWN. Only append-only MILESTONE-6 and DEVELOPMENT evidence updates; no git/gh, `.git` access, cleanup, system adjustment, archive or publishing. Stop for controller review; no Prompt 11B.
