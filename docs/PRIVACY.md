...
Prompt 9 performs no reachability check, metadata fetch, or background dispatch. A search term or HTTPS destination leaves Book Atlas only after a visible user action and disclosure that an external application may process it. UI presents a validated ASCII host rather than a full private URL. Local-file access begins only for selection or an explicit open, refreshes stale authorization visibly, and balances every successful security-scope start with a stop. CSV and Markdown exclude external URLs, local paths, and bookmark bytes; full backups necessarily preserve them as private library data.
