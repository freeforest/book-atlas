...
Prompt 9 production and repository tests use fixed temporary files, opaque fictional bytes, and fake selectors/bookmark/opening services. They cover cancellation, persistence, the 1 MiB boundary, oversized create/update rejection with the original record retained, stale refresh, move/missing/corrupt/revoked handling, re-selection, removal, canonical display-name rejection, cascading deletion, merge migration/rollback, backup/restore preservation, pre-BLOB backup rejection, streamed multi-row validation, and balanced scope access without opening a real user file. Prompt 9 remains awaiting another independent review after its first-review closure.
