...
- Prompt 9 adds book-scoped, generation-arbitrated HTTPS CRUD, explicit Apple Books/public-search/clipboard degradation, and read-only app-scoped local-file bookmarks behind focused replaceable integration protocols. Duplicate candidates use a separate read-only entry scope. Raw/decoded controls and malformed authority ports are rejected; bookmarks are capped at 1 MiB and strict restore validation checks length before streamed BLOB decoding. The nested duplicate-review hierarchy now has one active Escape owner, so candidate return cannot race with review cancellation or editor discard. It introduces Schema 5 only for opaque local-file references, keeps backup format 1, and extends strict restore validation through schemas 1–5.
- Prompt 9 passed independent review at baseline `1f7a35cda11fcafd23aacab0cb5c72e811327d0b`: Debug succeeded, 171/171 tests passed, and all 26 UI tests executed and passed. Prompt 10 is implemented and awaits independent review.
