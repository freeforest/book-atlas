...
- Prompt 9 adds revalidated HTTPS CRUD, explicit Apple Books/public-search/clipboard degradation, and read-only app-scoped local-file bookmarks behind focused replaceable integration protocols. It introduces Schema 5 only for opaque local-file references, keeps backup format 1, and extends strict restore validation through schemas 1–5. Prompt 9 is implemented and waiting for independent review; Prompt 10 has not started.
