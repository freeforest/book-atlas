...
- **Prompt 11A：手动书籍关系用户闭环。** 当前状态：**Prompt 11A 功能验收通过；未发布；用户 Git 与完整待提交范围核对待完成。**
- 主控已通过功能验收；裁决与证据边界见文末 2026-09-09 DOC-CLOSEOUT。原始门禁及带日期执行记录保留为历史，不代表当前执行授权或发布许可。
- **Prompt 11B：BookKind 与详情完整性。** 后续已获新授权，实施与限定验证见 [独立记录](PROMPT-11B.md)。以下 P11A 范围及带日期记录保持原样；P11 整体待主控最终验收。
...
## 2026-09-09 — MINIMAL-MANUAL-CHECK

人工检查使用上一节 MINIMAL-RELEASE-CHECK 已核验的 `DerivedData/Build/Products/Release/BookAtlas.app` 产物（本机唯一证据目录由交付记录定位）。提供给用户的独立实例启动方式如下；未使用保存等待替身开关，也未启动默认真实书库：

```sh
open -n "<RELEASE_EVIDENCE_DIR>/DerivedData/Build/Products/Release/BookAtlas.app" --args -ApplePersistenceIgnoreState YES -BookAtlasUseInMemoryStore -BookAtlasSeedManualRelationUITestData
```

应用户要求，一次提供全部检查步骤，由用户实际操作；用户随后统一反馈“确认均无异常”。以下结果依据该用户反馈记录，非代理自动操作或独立 UI 观测：

- 虚构环境确认：A101、B202、C303 可见，无启动异常。
- 创建：PASS（用户确认）。从 A101 指向 B202，类型“相关”、备注“人工验收”；保存前方向正确，保存后位于 A101 传出关系。
- 导航：PASS（用户确认）。点击新关系进入 B202，详情传入关系来自 A101，类型和备注正确。
- 删除：PASS（用户确认）。首次发起后取消，关系仍在；再次确认删除后该关系消失，A101 与 B202 两本书仍存在。

本次只证明上述 Release 产物、虚构内存环境下的人工交互；真实数据库持久性 UNTESTED，Intel 实机运行 NOT VERIFIED。历史完整 UI 仍为 44/45，修正用例独立 1/1，不合并为单轮 45/45；完整 UI 后续裁决、完整待提交范围及 Git 核对仍由主控/用户接管，保持 PENDING。Prompt 11A 尚未整体验收。

本轮仅追加本计划，未构建、未运行自动测试、未修改代码或工程配置；未执行 git/gh、未访问 `.git`，无清理、系统调整、提交或发布。**手动关系闭环的最小人工检查通过，等待主控最终验收。** 停止，不开始 Prompt 11B。

## 2026-09-09 — DOC-CLOSEOUT：主控最终裁决

主控明确裁决：**Prompt 11A 功能验收通过；未发布；用户 Git 与完整待提交范围核对待完成。** 本轮仅同步文档，不新增构建、测试或运行证据。

验收依据为 Store 定向 45/45、完整非 UI 221/221、包含保存忙碌态的关系专项 UI 4/4、Release 构建与最小产物核验，以及用户实际确认的人工闭环。

最终 UI 验收采用历史完整运行记录 **44/45** 加已定位失败项修复后的独立 **1/1**。唯一失败为数量文本期望遗漏分页外定位提示，主控接受其窄修复验证，不再要求因此重跑完整 UI。该裁决不是单轮完整 UI 45/45；历史失败、停止、退出码和未验证记录原样保留。

人工检查使用已核验的 Release 虚构内存独立实例，用户确认创建、导航、取消删除后关系保留、确认删除后关系消失且两本书仍在，均无异常。此证据不扩大为真实用户数据库持久性或 Intel 实机运行验证。

原始 Gates 清单保留供追溯：第 6 项 UI 的最终验收按上述主控裁决处理；第 9 项人工环境采用已核验 Release 虚构内存实例。用户 Git 和完整待提交范围核对仍为 **PENDING**，包括原清单中的用户 Git 检查，不标为通过。真实用户数据库持久性的人工验证仍 **UNTESTED**，Intel 实机运行仍 **NOT VERIFIED**；旧连接中断、授权错误、丢字符及取消的根因继续分别 **UNKNOWN**。

功能验收不等于完整待提交范围审计、提交或发布。Prompt 11B 未授权，不开始或预留实现。本轮只核对授权文档的状态一致性、引用和明显格式问题；未执行 git/gh、未访问 `.git`，未修改代码、构建、运行测试、清理、提交、推送、打标签或发布。文档同步完成后停止。
