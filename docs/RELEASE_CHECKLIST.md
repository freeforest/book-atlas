...
surface checks. The source-only V1.0.0 Release was published and publicly
verified on 2026-08-10. Repository operations require explicit owner
...
The Release uses the tag's GitHub-generated source archives and prepared notes
only. It has zero custom assets and no precompiled `.app`, `.dmg`, `.pkg`,
binary application archive, certificate, or signing material. Mac App Store,
Apple Developer, Developer ID, notarization, stapling, and Gatekeeper binary
distribution are not part of this source-publication task.
...
source publication is a separate operational task and is not Prompt 11.
...
- [x] Repository became Public on 2026-08-10; logged-in and independent public
      ...
      remained `main`.
- [x] Public source version is 1.0.0; annotated tag is `v1.0.0`; published
      Release title is `Book Atlas v1.0.0`.
- [x] Publication is source-only and users build with a compatible Xcode 26
      toolchain.
- [x] No precompiled application or binary package is included in V1.0.0.
...
- [x] `docs/RELEASE_NOTES-1.0.0.md` removes the draft state and is finalized
      for 2026-08-10 without a private/test-temporary path; the published
      Release body matches this file.
- [x] Confirm `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, Issue/PR templates,
      SampleData, Scripts, roadmap, quality, privacy, and security materials
      were reviewed for the source release; the operational PVR entry and the
      separate conduct-reporting channel remain independently verified.
...
- [x] Create annotated tag `v1.0.0` at
      `991d932a5fafe0d9821b46ec5e779cd6c9849171` under explicit
...
non-draft, non-prerelease, source-only GitHub Release. The Release body matches
the prepared notes, the custom asset list is empty, and GitHub supplies only its
generated source archives. The optional announcement remains unchecked and is
not a release blocker. Authentication challenges remain owner-only.
