...
Prompt 7 backups use `sqlite3_backup` against the live connection, convert the completed temporary snapshot to single-file DELETE journal mode, add a path-free manifest, verify `PRAGMA integrity_check`, and refuse existing destinations. Restore rejects symbolic links, non-regular or wrong-type files, invalid SQLite headers/manifests, integrity failures, and future format/schema versions. It creates and validates a recovery snapshot, stages and migrates the selected database, closes the live connection, replaces it, and reopens it. Failure injection covers interruption, replacement, rollback, and reconnect boundaries. Details are in ADR-0008 and `docs/FORMATS/PORTABILITY.md`.
