...
- **Prompt 11B:** now separately authorized for BookKind editing, detail and
  filtering; implementation exists, the two corrected non-UI tests passed; the UI batch is BLOCKED (1 passed, 1 failed). See
...
Prompt 11B is BLOCKED after the UI batch failed; no further product stage or release is authorized.
