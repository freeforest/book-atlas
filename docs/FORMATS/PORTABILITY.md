...
Markdown starts with its format version and UTC export time, then contains each book's bibliographic fields, reading state, dates, tags, collections, sources, and note. Markdown control characters are escaped, multiline values use `<br>`, and absolute local path patterns are replaced by a local-path omission marker. External URLs, local display names and paths, and bookmark/authorization values are not exported. Book Atlas does not execute links or automatically open the output.
...
Restore requires a regular, non-symbolic-link `.bookatlasbackup` with the SQLite header, manifest version 1, a supported schema in 1–5, matching `PRAGMA user_version`, and a successful application-schema check. That check requires `schema_migrations` to contain exactly every version through `user_version`; verifies the exact versioned set of tables, ordered required columns, primary keys, unique keys, foreign keys, cascade actions, and critical checks; requires `PRAGMA foreign_key_check` to return no rows; and decodes every book, organization, join, external link, manual relation, duplicate index/token row, ignored pair, and Schema 5 local-file reference through domain types. Schema 5 first checks each opaque bookmark with SQLite `length()` and rejects zero or more than 1,048,576 bytes before reading the BLOB; valid rows are then streamed one at a time. Stored display names must already be canonical: nonempty, trimmed, at most 512 characters, free of C0/DEL controls and `/` or `\\`, and not `.` or `..`. Invalid UUIDs, dates, enums, normalized duplicate keys, noncanonical display names, invalid bookmark BLOBs, orphaned rows, or missing structures are rejected.
...
Prompt 7 introduced file-format versions without changing the then-current Schema 4. Prompt 9 advances the application schema to 5 for opaque local-file references while keeping CSV, Markdown, and backup format versions at 1. Schema 5 backups preserve private URLs and opaque bookmark bytes for full fidelity; the interchange exports intentionally omit them.
